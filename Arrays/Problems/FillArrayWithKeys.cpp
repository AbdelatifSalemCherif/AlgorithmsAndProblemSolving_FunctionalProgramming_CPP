...
namespace FillArrayWithKeys
{
    ...
    void GenerateNKeys(string Arr[100], int NumberOfKeys, int NumberOfWord, int NumberOfCharacterForaWord, enCharacterType CharacterType)
    {
        cout << endl;

        for (int i = 0; i < NumberOfKeys; i++)
        {
            Arr[i] = RandomKey(CharacterType, NumberOfCharacterForaWord, NumberOfWord);
        }

        cout << endl;
    }

    void PrintArrayKeys(string ArrayKeys[100], int NumberOfKeys, string Header, string WordBetweenElements)
    {
        cout << Header;

        for (int i = 0; i < NumberOfKeys - 1; i++)
        {
            cout << "Array [" << i << "] : " << ArrayKeys[i] << WordBetweenElements;
        }

        cout << "Array[" << NumberOfKeys - 1 << "] : " << ArrayKeys[NumberOfKeys - 1];
    }
    ...
    void Run()
    {
        ...
        int NumberOfKeys = ReadPositiveNumber();

        GenerateNKeys(ArrayKeys, NumberOfKeys, 4, 4, enCharacterType::CapitallLetter);

        PrintArrayKeys(ArrayKeys, NumberOfKeys, "\n\nArray elements : \n\n", "\n");
