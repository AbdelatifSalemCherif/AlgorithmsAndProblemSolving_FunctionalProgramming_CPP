...
namespace DateFromDayOrderInAYear
{
    ...
    void Run()
    {
        ...
        int ThisDayOrderInYear = DayOrderInYear(Year, Month, Day);

        MyOutput::PrintNumber("\nNumber of Dayes from the beginning of the year is ", ThisDayOrderInYear, "\n");

        DateFromDayOrderInYear(Year, ThisDayOrderInYear, Month, Day);

        cout << "\nDate for[" << ThisDayOrderInYear << " is : " << Day << "/" << Month << "/" << Year << endl;
